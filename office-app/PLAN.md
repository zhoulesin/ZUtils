...
| **P1: 信息查询** | `base64` 注册、天气/翻译 MCP 对接、`runQueryRaw` MCP 自动路由 | bootstrap + AgentExecution |
...
| 类别 | 函数 |
|------|------|
| 设备信息 | `getDeviceInfo`, `getScreenInfo`, `getStorageInfo`, `getNetworkType` |
| 工具 | `getClipboard`, `setClipboard`, `base64` |
| 文件 | `readFile`, `writeFile`, `shareFile` |
| 通信 | `queryContacts`, `makePhoneCall`, `sendSms` |
| 日历 | `createCalendarEvent`, `queryCalendarEvents` |
...
| 路径 | 触发条件 | 能力 |
|------|---------|------|
| **关键词** | 无 LLM / LLM 不可用时 | 17 个本地函数 + MCP 天气/翻译 |
| **LLM** | `ServerLlmClient` 可用时 | 复杂多步推理 + 函数调用链 |
| **MCP** | 关键词或 LLM 路由到远程工具 | 天气/翻译/新闻/搜索/邮件/文档等 |
